...
//
// A THREE-WAY MIRROR of the RN SDK's lib/poa-country-gate.ts and Flutter's
// config/poa_country_gate.dart; change the rule in one and change all three.

/** What the address-scope country control offers: the org's accepted list
 *  (`proofOfAddress.countries`, unknown codes dropped), else every country. */
export function poaOfferedCountries(configured: readonly string[] | undefined): string[] {
  const list = (configured ?? []).map((c) => c.toUpperCase()).filter((c) => ALL_REGION_CODES.includes(c));
