...
import type { BiometricCopy, BiometricCopyText } from './biometric-options';
...
// describeOutcome (lib/result-copy.ts) as overrides. Mirrors the RN SDK's
...
export function biometricCopyFor(config: CopyConfigLike): ResolvedBiometricCopy {
  ...
  const tokens = { firstName: config.userData?.firstName, lastName: config.userData?.lastName };
  const fill = (text: BiometricCopyText | undefined): BiometricCopyText | null => {
    if (!text) return null;
    const title = text.title ? fillTokens(text.title, tokens) : '';
    const description = text.description ? fillTokens(text.description, tokens) : '';
