...
import { fitStepCircles, windowedSteps } from './step-window';
...
// Representative row widths. These exercise the pure fitting maths directly, so
// they are deliberately the same numbers the RN and Flutter suites use (a phone
// viewport less 16px of padding a side) rather than tracking the web header's
// own px-6 — the point is that all three SDKs agree on the function.
const SMALL_ANDROID = 360 - 32; // 328
const IPHONE_PRO_MAX = 430 - 32; // 398
const TINY = 320 - 32; // 288

describe('fitStepCircles', () => {
  it('fits more circles on wider viewports', () => {
  ...
  it('reports 0 for an unmeasured width, which means "render them all"', () => {
    // The first frame has no measurement yet; it must not flash a collapsed row.
    expect(fitStepCircles(0, 26)).toBe(0);
    expect(fitStepCircles(NaN, 26)).toBe(0);
    expect(fitStepCircles(-100, 26)).toBe(0);
  });

  it('fits fewer circles once a larger base font size grows them', () => {
...
describe('windowedSteps', () => {
  ...
  it('collapses only once the steps would stop fitting', () => {
    // The same 9-step flow at two widths: the wider one shows all of it, the
    ...
    // And a genuinely long one collapses at both, because it fits at neither.
  ...
  it('matches the React Native and Flutter SDKs exactly', () => {
    // The three SDKs must window identically or the same workflow looks
    // different depending on which one the customer embedded. These are the
    // same assertions as stepWindow.test.ts (RN) and step_window_test.dart.
    expect(render(10, 5, 8)).toBe('1 … 5 6 7 8 … 10');
    expect(render(12, 0, 7)).toBe('1 2 3 4 … 12');
    expect(render(12, 11, 7)).toBe('1 … 9 10 11 12');
    expect(render(9, 4, 8)).toBe('1 … 4 5 6 7 … 9');
    expect(render(14, 6, 9)).toBe('1 … 5 6 7 8 9 … 14');
  });
});
