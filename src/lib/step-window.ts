 ...
 * to fit: on a 360px viewport with 16px padding, ten steps leave 14px TOTAL for
 * nine connectors, and twelve steps overflow the row outright.
 *
 * COLLAPSING IS A LAST RESORT. The row measures itself and shows as many real
 * circles as the width allows, windowing only once they would stop reading as a
 * connected chain — which is what the minimum connector length below decides. A
 * fixed cap would collapse a 9-step flow on a desktop that had room for all of
 * it.
 *
 ...
 *     1 ··· 5 6 7 ··· 10
 *
 * Byte-for-byte the same algorithm as the React Native SDK's lib/step-window.ts
 * and the Flutter SDK's widgets/step_window.dart. It is pure on purpose: the
 * three SDKs must window a given flow identically, or the same workflow looks
 * different depending on which one the customer embedded. Change one, change
 * all three, and keep the shared parity assertions in their test suites green.
 */
